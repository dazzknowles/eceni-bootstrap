...
- [NVIDIA App](https://www.nvidia.com/en-gb/software/nvidia-app/) and [NVIDIA Drivers](https://www.nvidia.com/en-gb/drivers/): the app uses NVIDIA's official installer and hardware-specific Studio driver selection remains manual. The [Nvidia namespace in the WinGet repository](https://github.com/microsoft/winget-pkgs/tree/master/manifests/n/Nvidia) does not contain an NVIDIA App manifest, so the bootstrap does not invent a package ID.
